...
khrtime hrtime_offset;
khrtime hrtime_basis = init_hrtime_basis();
...
#endif
  do {
    t1 = kget_hrtime_internal();
#if HAVE_CLOCK_GETTIME
    kassert(!clock_gettime(CLOCK_REALTIME, &timespec_basis));
#else
    {
      struct timeval tnow;
      kassert(!gettimeofday(&tnow, nullptr));
      timespec_basis.tv_sec  = tnow.tv_sec;
      timespec_basis.tv_nsec = tnow.tv_usec * 1000;
    }
#endif
    t2 = kget_hrtime_internal();
    // accuracy must be at least 100 microseconds
  } while (t2 - t1 > HRTIME_USECONDS(100));
  b   = (t2 + t1) / 2;
  now = khrtime_from_timespec(&timespec_basis);
  ts  = khrtime_to_timespec(now);
  kassert(ts.tv_sec == timespec_basis.tv_sec && ts.tv_nsec == timespec_basis.tv_nsec);
  hrtime_offset = now - b;
  hrtime_basis  = b;
  return b;
}
