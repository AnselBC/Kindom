...
void kfree(void *ptr);
void *kfree_null(void *ptr);
void kmemalign_free(void *ptr);
int kmallopt(int param, int value);

int kmsync(caddr_t addr, size_t len, caddr_t end, int flags);
int kmadvise(caddr_t addr, size_t len, int flags);
int kmlock(caddr_t addr, size_t len);

void *ktrack_malloc(size_t size, uint64_t *stat);
void *ktrack_realloc(void *ptr, size_t size, uint64_t *alloc_stat, uint64_t *free_stat);
void ktrack_free(void *ptr, uint64_t *stat);

static inline size_t __attribute__((const)) kpagesize(void)
{
...
#define kstrdup(p) _xstrdup((p), -1, nullptr)
#define kstrndup(p, n) _xstrdup((p), n, nullptr)
...
/** Set data to zero.
    ...
    foo *bar; // pointer.
    kzero(bar); // WRONG - makes the pointer @a bar zero.
    kzero(*bar); // zero what bar points at.

    foo bar[ZOMG]; // Array of structs.
    kzero(bar); // Zero all structs in array.

    foo *bar[ZOMG]; // array of pointers.
    kzero(bar); // zero all pointers in the array.
    @endcode
...
public:
  ...
  kscoped_resource() : _r(Traits::initValue()) {}
  /// Construct with contained resource.
  explicit kscoped_resource(value_type rt) : _r(rt) {}
  /// Destructor.
  ~kscoped_resource()
  {
...
namespace detail
{
/** Traits for @c kscoped_resource for file descriptors.
 */
 ...
 */
class kscoped_fd : public kscoped_resource<detail::SCOPED_FD_TRAITS>
{
public:
  typedef kscoped_resource<detail::SCOPED_FD_TRAITS> super; ///< Super type.
  typedef kscoped_fd self;                                  ///< Self reference type.

  /// Default constructor - an empty container.
  kscoped_fd() : super() {}
  /// Construct with contained resource.
  explicit kscoped_fd(value_type rt) : super(rt) {}
  /** Place a new resource @a rt in the container.
      Any resource currently contained is destroyed.
      This object becomes the owner of @a rt.
  */
...
namespace detail
{
/** Traits for @c kscoped_resource for pointers from @c kmalloc.
 */
...
/// Traits for @c kscoped_resource for objects using @c new and @c delete.
template <typename T ///< Underlying type - not the pointer type.
          >
...
/** Specialization of @c kscoped_resource for strings.
    This contains an allocated string that is cleaned up if not explicitly released.
*/
class kscoped_str : public kscoped_resource<detail::SCOPED_MALLOC_TRAITS<char>>
{
public:
  typedef kscoped_resource<detail::SCOPED_MALLOC_TRAITS<char>> super; ///< Super type.
  typedef kscoped_str self;                                           ///< Self reference type.

  /// Default constructor (no string).
  kscoped_str() {}
  /// Construct and allocate @a n bytes for a string.
  explicit kscoped_str(size_t n) : super(static_cast<char *>(kmalloc(n))) {}
  /// Put string @a s in this container for cleanup.
  explicit kscoped_str(char *s) : super(s) {}
...
/** Specialization of @c kscoped_resource for pointers allocated with @c kmalloc.
 */
template <typename T ///< Underlying (not pointer) type.
          >
class kscoped_mem : public kscoped_resource<detail::SCOPED_MALLOC_TRAITS<T>>
{
public:
  typedef kscoped_resource<detail::SCOPED_MALLOC_TRAITS<T>> super; ///< Super type.
  typedef kscoped_mem self;                                        ///< Self reference.
...
/** Specialization of @c kscoped_resource for objects.
    This handles a pointer to an object created by @c new and destroyed by @c delete.
*/

template <typename T /// Underlying (not pointer) type.
          >
class kscoped_obj : public kscoped_resource<detail::SCOPED_OBJECT_TRAITS<T>>
{
public:
  typedef kscoped_resource<detail::SCOPED_OBJECT_TRAITS<T>> super; ///< Super type.
  typedef kscoped_obj self;                                        ///< Self reference.

  /// Default constructor - an empty container.
  kscoped_obj() : super() {}
  /// Construct with contained resource.
  explicit kscoped_obj(T *obj) : super(obj) {}
  self &
  operator=(T *obj)
  {
    super::operator=(obj);
...
/** Combine two strings as file paths.
     ...
     @return A newly @x kmalloc string of the combined paths.
*/
inline char *
path_join(kscoped_str const &lhs, kscoped_str const &rhs)
{
